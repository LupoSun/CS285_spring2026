...
class FQLAgent(nn.Module):
    ...
    def update_q(
        ...
        with torch.no_grad():
            ...
            target_qs = self.target_critic(next_observations, next_actions)
            target_q = target_qs.mean(dim=0)
            y = rewards + self.discount * (1 - dones) * target_q
        qs = self.critic(observations, actions)
        q = qs
        loss = ((qs - y[None]) ** 2).sum(dim=0).mean()
    ...
    def update_bc_actor(
        ...
        a_tilde = (1-t) * noise + t * actions
        target_velocity = actions - noise

        predicted_velocity = self.bc_actor(observations, a_tilde, t)
        loss = ((predicted_velocity - target_velocity) ** 2).mean(dim=-1).mean()
    ...
    def update_onestep_actor(
        ...
        mse = ((onestep_actions_clipped.detach() - actions) ** 2).mean(dim=-1).mean()
