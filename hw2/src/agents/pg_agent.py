...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discount = self.gamma ** indices
        total_return = np.sum(discount * rewards)

        return np.full(len(rewards), total_return)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            tpu_tensor = ptu.from_numpy(obs)
            values = ptu.to_numpy(self.critic(tpu_tensor))
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    next_val = 0 if terminals[i] == 1 else values[i+1]
                    next_adv = 0 if terminals[i] == 1 else advantages[i+1]
