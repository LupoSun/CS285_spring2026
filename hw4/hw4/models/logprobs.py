...
def compute_per_token_logprobs(
    ...
    *,
    ...
    with torch.set_grad_enabled(enable_grad):
        ...
        loss = F.cross_entropy(shift_logits.flatten(0, 1), shift_labels.flatten(0, 1), reduction='none')

        logprobs = -loss.view(B, L_minus_1)
        return logprobs
...
def build_completion_mask(
    ...
    is_completion_pos = (indices >= prompt_input_len-1)
    is_not_padding = (attention_mask[:, 1:] == 1)    
    mask = (is_completion_pos & is_not_padding).float()
    return mask
...
def approx_kl_from_logprobs(
    ...
    delta = torch.clamp(log_ratio, min=-log_ratio_clip, max=log_ratio_clip)
