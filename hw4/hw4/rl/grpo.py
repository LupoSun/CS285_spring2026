    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                adv_reshaped = adv.unsqueeze(1)
                
                unclipped_obj = ratio * adv_reshaped
                
                clipped_obj = torch.clamp(ratio, 1 - cfg.clip_eps, 1 + cfg.clip_eps) * adv_reshaped

                per_token_obj = torch.min(unclipped_obj, clipped_obj)

                seq_obj = masked_mean_per_row(per_token_obj, mb.completion_mask, eps=1e-8)

                pg_loss = -seq_obj.mean()
                
                kl = approx_kl_from_logprobs(new_logp, mb.ref_logprobs, mask)
                entropy = -masked_mean(new_logp, mask)
                is_clipped = (ratio<1-cfg.clip_eps) | (ratio>1+cfg.clip_eps)
                clipfrac = (is_clipped.float() * mask).sum() / (mask.sum() + 1e-8)
